...
if __name__ == '__main__':
    train_args = {
        ...
        'use_n_graphs': True, #if True, the number of graphs the sample will be used in the decoder (overrides use_extra_graphs)
        'n_epochs': 100,
    ...
    args = parse_args()
    for problem_name in train_args['problem_name']:
        ...
        for problem_type in ['classification', 'regression']:
            ...
            for network_type in ['no_extra', 'n_graphs', 'extra_graphs']:
                if network_type == 'no_extra':
                    train_args['use_extra_graphs'] = False
                    train_args['use_n_graphs'] = False
                elif network_type == 'n_graphs':
                    train_args['use_extra_graphs'] = False
                    train_args['use_n_graphs'] = True
                elif network_type == 'extra_graphs':
                    train_args['use_extra_graphs'] = True
                    train_args['use_n_graphs'] = True
                ...
                if train_args['problem_name'] == 'prepaid':
                    if train_args['generate_dataset']:
                        ...
                        graph_dataset_aggregator = GraphDatasetAggregator(instance_graph_file_path,
                                                                          sub_instance_graphs_folder_path,
                                                                          log_file_path, problem_type=train_args[
                                'problem_type'])  # , n_samples=n_samples)
                        ...
                        if train_args['reduced_dataset']:
                            with open(
                                    f"data/graphs_minimal_{train_args['problem_name']}_{train_args['problem_type']}.pkl",
                                    'wb') as file:
                                pickle.dump(graphs, file)
                        else:
                            with open(f"./data/graphs_{train_args['problem_name']}_{train_args['problem_type']}.pkl",
                                      'wb') as file:  # with open('data/graphs_minimal.pkl', 'wb') as file:
                                pickle.dump(graphs, file)
                    else:
                        if train_args['reduced_dataset']:
                    ...
                    if train_args['generate_dataset']:
                        ...
                        graphs = get_graphs(
                            f"./data/graphs_{train_args['problem_name']}_{train_args['problem_type']}.pkl")
                    ...
                    if train_args['generate_dataset']:
                        ...
                        graph_dataset_aggregator = GraphDatasetAggregator(instance_graph_file_path,
                                                                          sub_instance_graphs_folder_path,
                                                                          log_file_path,
                                                                          target_column_name='remaining_time_hours',
                                                                          problem_type=train_args['problem_type'])
                        graph_dataset_aggregator.aggregate()
                        graphs = graph_dataset_aggregator.parse_graph_with_subgraphs_file()
                ...
                for epoch in range(train_args['n_epochs']):
                    ...
                    for batch in simple_batching(val_dataset, 1):
                        ...
                        if train_args['problem_type'] == 'regression':
                            ...
                            #out = denormalize_predictions(out.squeeze(), target_mean, target_std)
                            val_loss = torch.nn.functional.mse_loss(out.squeeze(), batch_y)
                        elif train_args['problem_type'] == 'classification':
                            batch_y = torch.stack([torch.tensor(one_hot_encoding_targets(model.unique_classes, graph.y)) for graph in batch['current_graph']]).float()
