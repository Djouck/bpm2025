...
def add_start_end_activities(df):
    """
    Add start and end activities to an event log dataframe with guaranteed ordering.

    Parameters:
    df (pandas.DataFrame): DataFrame with columns 'case_id', 'activity', 'timestamp'

    Returns:
    pandas.DataFrame: Original data with additional start and end activities
    """
    # Make a copy to avoid modifying the original
    result_df = df.copy()

    # Ensure timestamp is datetime
    result_df['time:timestamp'] = pd.to_datetime(result_df['time:timestamp'])

    # Create a new DataFrame for start and end activities
    start_activities = []
    end_activities = []

    # Get unique case IDs
    case_ids = result_df['case:concept:name'].unique()

    for case_id in case_ids:
        # Get data for this case
        case_data = result_df[result_df['case:concept:name'] == case_id]

        # Get first and last timestamps
        first_timestamp = case_data['time:timestamp'].min()
        last_timestamp = case_data['time:timestamp'].max()
        ...
        start_activities.append({
            ...
            'artificial_order': 0  # This will ensure it comes first
        })
        ...
        end_activities.append({
            ...
            'artificial_order': 2  # This will ensure it comes last
        })

    # Add artificial_order column to original data (middle position)
    result_df['artificial_order'] = 1

    # Convert to DataFrames
    start_df = pd.DataFrame(start_activities)
    end_df = pd.DataFrame(end_activities)

    # Combine with original data
    combined_df = pd.concat([result_df, start_df, end_df], ignore_index=True)

    # Sort with the additional artificial_order field to guarantee proper ordering
    combined_df = combined_df.sort_values(['case:concept:name', 'time:timestamp', 'artificial_order'])

    # Optionally remove the artificial_order column if not needed
    combined_df = combined_df.drop('artificial_order', axis=1)
...
input_file_path = 'BPIC15_1_Final_Data_File_Reg.csv'
...
df.rename(columns={"Case ID": "case:concept:name", 'Activity': 'concept:name'}, inplace=True)
df = add_start_end_activities(df).reset_index()
...
for index, r in df.iterrows():
    ...
    act = r['concept:name'].strip().replace(' ', '')
...
with open('BPIC15_1_rightOrder.g', 'r') as file:
